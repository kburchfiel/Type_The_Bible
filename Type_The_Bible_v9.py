...
# * Consider updating your autosave settings to save after every 10th (or even 20th) test rather than after every test.
# * Revise file names or create a renaming script so that users won't need to manually rename them.
# * Add in analyses that show which words are mistyped the most frequently.
        ...
        if test_type == 'v1': 
            ...
            # # {(character_timestamp_list[-1] - character_timestamp_list[-3]) / 1000000} ms.")
                ...
                if verse_response == verse: # Note that, unlike with version
                    ...
                    # print("Character execution times (in ms)",
...
# %%
df_results, df_character_stats, df_word_stats, run_analyses = run_game(results_table = df_results, character_stats_table=df_character_stats, word_stats_table = df_word_stats)
...
Word Stats, and Bible .csv files.")
...
# %%
fig_backspaces_wpm_scatter = px.scatter(df_results, 
x = 'Backspaces as % of Verse Length', y = 'WPM', trendline = 'ols')
# Note that you can hover over the best fit line to see the regression results.
fig_backspaces_wpm_scatter.write_html('Analyses/accuarcy_wpm_scatter.html')
fig_backspaces_wpm_scatter.write_image('Analyses/accuarcy_wpm_scatter.png', 
width = 1920, height = 1080, engine = 'kaleido', scale = 2)
fig_backspaces_wpm_scatter

# %%
fig_accuracy_wpm_histogram = px.histogram(df_results, x = 'Backspaces as % of Verse Length', y = 'WPM', histfunc = 'avg', nbins = 20, text_auto = '.6s')
fig_accuracy_wpm_histogram.update_layout(bargroupgap = 0.1)
fig_accuracy_wpm_histogram.write_html('Analyses/accuracy_wpm_histogram.html')
fig_accuracy_wpm_histogram.write_image('Analyses/accuracy_wpm_histogram.png', 
width = 1920, height = 1080, engine = 'kaleido', scale = 2)
fig_accuracy_wpm_histogram


# %%
fig_accuracy_count_histogram = px.histogram(df_results, x = 'Backspaces as % of Verse Length', nbins = 20, text_auto = True)
fig_accuracy_count_histogram.update_layout(bargroupgap = 0.1)
fig_accuracy_count_histogram.write_html('Analyses/accuracy_count_histogram.html')
fig_accuracy_count_histogram.write_image('Analyses/accuracy_count_histogram.png', 
width = 1920, height = 1080, engine = 'kaleido', scale = 2)
...
# %% [markdown]
# Note: This section used to include code that would calculate the fastest single-character and character-pair keypress times. However, I found that these results could be distorted by computer lag. Specifically, if the player enters keys while the computer is frozen, some of those keypress durations will be reported as shorter than they actually were. (This makes sense, in that while the computer is frozen, the actual keypress time might not get logged; instead, a later post-freeze time will get logged. This post-freeze time will then be closer to the next keypress time, causing a shorter-than-actual keypress time to get reported.) (I simulated computer lag in my tests by entering a time.sleep() command, but I believe actual lag would produce similar errors.)
# 
# Therefore, while the code still shows median keypress times (which are less susceptible to this issue), it will no longer calculate the fastest individual keypress times. These can still be located within the character stats file, but the results should be interpreted with great caution.

# %%
# If df_character_stats is empty, we'll need to skip character analyses
# in order to avoid errors. The following code facilitates this.

if len(df_character_stats) == 0:
    run_character_analyses = 0
else:
    run_character_analyses = 1

run_character_analyses

# %%
if run_character_analyses == 1:
    df_character_durations = df_character_stats.pivot_table(index = 'Character', values = ['Time Used to Type Last Character (ms)', 'Count'], aggfunc = {'Time Used to Type Last Character (ms)':'median', 'Count':sum}).reset_index().sort_values('Time Used to Type Last Character (ms)').reset_index(drop=True)
...
# %%
if run_character_analyses == 1:
    df_last_2_character_durations = df_character_stats.pivot_table(index = 'Last 2 Characters', values = ['Time Used to Type Last 2 Characters (ms)', 'Count'], aggfunc = {'Time Used to Type Last 2 Characters (ms)':'median', 'Count':'sum'}).reset_index().sort_values('Time Used to Type Last 2 Characters (ms)').reset_index(drop=True)
...
# %%
# We'll also skip the word-level analyses section if we have no data to analyze.
...
# %%
if run_word_analyses == 1:
    fig_words_with_highest_median_wpms = px.bar(df_word_stats_pivot.head(50), 
...
if run_word_analyses == 1:
    ...
    'Median WPM'), x = 'word', y = 'Median WPM', color = 'Count', text_auto = '.6s')
...
# %% [markdown]
# This script used to show the highest-ever WPMs for individual words, but for reasons explained during the character stats section, this code has been removed due to accuracy issues.
